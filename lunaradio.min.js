...
$(document)['ready'](function() {
  ...
  function setOffAir() {
      $('#livebtn')['html']('Off-Air');
      if ($('#livebtn')['hasClass']('onair')) {
          $('#livebtn')['removeClass']('onair')['addClass']('offair')
      };
      $('#titletxt')['html']('Radio Offline');
      $('#artisttxt')['html']('N/A');
      $('#albumtxt')['html']('');
      $('.blur')['attr']('src', 'img/nocover.png');
      $('.neontitle')['resetKeyframe']();
      animationactive = 0
  }
var url = ''+$radioURL+'/stats?sid=1&json=0&callback=?';
  localStorage['myCover'] = "";
  var audiostream = $('.cue-audio').attr('src')
  $('#radioSong')['text']('');
  ...
  function foo() {
    doCORSRequest({
        ...
        data: dataField.value*/
          ...
          //console.log(result);
          var startPoint=result.indexOf("<SONGTITLE>")+11;
          var lengthPoint=result.indexOf("</SONGTITLE>")-startPoint;
          var curSong=result.substr(startPoint,lengthPoint);

          var songtitleSplit = curSong.split(' - ');
          if ( songtitleSplit[0] == null ) songtitleSplit[0]="";
          if ( songtitleSplit[1] == null ) songtitleSplit[1]="";
          var artist = songtitleSplit[0];
          var title = songtitleSplit[1];
          $('#titletxt').html(title);
          $('#artisttxt').html(artist);
   ...
   function getCover(artist, title) {
    ...
    var myCover = localStorage['myCover'];
    if(myCover == ''){ 
      localStorage['myCover'] = $('.blur').attr('src'); 
    }		
        ...
        success:
            function(data) {						
                if (data.results.length == 1){							
                    ...
                    var cover = localStorage['myCover'];
            }
                $('.blur').attr('src', cover);
      },				
        error: 
...
function doCORSRequest(options, printResult) {
  ...
  function formatTitle(title){
    ...
    return title;
  $('.blur')['attr']('src', 'img/nocover.png');
                  if ( typeof (playing) != "undefined" && playing !== null && playing !== '') {
          $('#artisttxt').html('');
          $('#titletxt').html(playing);
          } else {
          $('#titletxt').html('N/A');
          $('#artisttxt').html('N/A');
