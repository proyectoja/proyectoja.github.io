...
fetch(jsonUrl) // PRODUCCIÓN
  ...
  .then((data) => {
    ...
    const soloValidas = data.peliJuntos.filter(
      (video) =>
    ...
    data.peliJuntos.sort((a, b) => b.fecha - a.fecha); // PRODUCCIÓN
    const fragment = document.createDocumentFragment();
    const generosSet = new Set();
    let peliculasValidas = [];

    data.peliJuntos.forEach((video) => {
...
function limpiarTodasLasURLs(data) {
  ...
  if (Array.isArray(data.peliJuntos)) {
    data.peliJuntos.forEach((pelicula) => {
...
function contadorDeVistas(cartel) {
  cartelAux = "pelisJuntos_" + cartel.id;
